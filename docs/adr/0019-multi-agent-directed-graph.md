...
Agent 主循环不是「被喂一句用户输入」，是**从收件箱取走此刻攒着的全部**：

```
pending = false;                        // 还有活干：别等新消息，直接跑下一圈
while (1) {                             // 一圈 = 一个 turn
    wait(pending || inbox 非空);        // 两样都没有 = idle，阻塞
    for (m : inbox.take_all())          // 每圈开头收一次件，可能一条也没有
        record_user(m);
    pending = false;
    ... 调 LLM ...
    if (没调工具) { 回一句「完成了就 stop，否则接着做」; pending = true; }
    else          { 顺序执行，结果入会话; pending = 这批里没有 stop;  }
}
```

**收工的判据是模型调 `stop` 工具，不是「它这次没调工具」**（2026-08-29 补）。原先拿后者当收工信号，等于把「一句话说完了」当成「活干完了」——这两件事本来就不是一回事，模型说一句「我先看看这个文件」也会触发收工。现在 `stop` 是一个普通工具（名字避开 `close`——那在这份 ADR 里已经是 agent 的一个状态），跟 `read`/`bash` 同一张表、同一条执行路径；loop 认的是结果里的 `"stop": true` 字段而不是工具名字，否则等于把工具表抄一份进循环里。

两个后果，都得认：

- **模型不调 stop 就一直跑。** 不设轮数上限（同下），刹车只有用户中断、LLM 调用失败、端点空回答三条。所以 `stop` 的契约必须写进 system prompt——模型不知道有这个出口就永远不会调它，那不是「一直做」，那是烧钱。
- **没调工具时 core 必须回一句。** 两个理由。协议上：不回的话下一次请求的 `messages` 以 assistant 结尾，那是「续写」不是「对话」，三套协议各按各的来，开了 thinking 的直接拒。行为上：模型对「结束」的意识不清晰，system prompt 里那一句它会忘，得每次把选择原样摆回它面前——**「完成了就打 `stop`，否则接着做」**。措辞上不说「你还没完成」：那是 core 替它判断活干没干完，而这件事只有它知道。这条 user 消息照常入会话、照常发 `message_start` 帧——实时与翻历史长得一样（[[ADR-0020]]）。

**只有一层循环，重复单位是 turn，不是消息。** 一条消息要多个 turn 是协议决定的：模型自己跑不了工具，它要调工具时就停在那儿（`stop_reason = tool_use`，HTTP 响应到此结束），工具结果只能作为**下一次请求**的输入送回去。于是 turn 数 = 要工具的次数 + 1。

「一趟」（`agent_start` 到 `agent_end`、跑完通知邻居、把历史还给盘、复位 `abort_`）**不是第二层循环**，是 idle ⇄ busy 那条边沿；代码里就是那把 `run_mtx_` 的持有与否（`busy.owns_lock()`），开跑上锁、收工解锁，不另存状态位。`pending` 写进等待条件里，于是「接着跑下一个 turn」与「等一条新消息」不是两条分支，是同一次等待的两种放行理由。

三种来源在主循环里不产生任何分支。**一口气全取走，取用点在 turn 边界。** 而「继续等」也不需要任何机制——就是这一轮不调工具，回去等。收件箱还有就接着跑，空了才真 idle。

两件事由这个形状直接得到，都不需要为它们再加状态：

- **跑着的时候投进来的消息，下一个 turn 就进得去。** 不必等整趟跑完——模型做完手上这一步就看见了新指令，这是插话该有的时延。
- **模型正在思考或正在跑工具时，没人来取。** 「等它做完」不是一条规则，是「唯一的取用点在 turn 边界」的后果：一次 LLM 调用中途插不进消息，因为那段时间根本没有 `drain()`。

修订（2026-08-29）：原文写的是「**一次取一条，不批量取走**——攒成一批等于 core 替它决定了『这几条要一起看』」。反了：那几条本来就是同时到的，一条一条喂才是替模型决定了「先只看这一条」，而它得看见全部才判断得了先做哪件；而且一条一条喂意味着第二条要等第一条整趟跑完（可能是几十轮工具调用），插话变得没用。
