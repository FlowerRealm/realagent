...
static ToolResult call(const std::string& name, const std::string& params) {
...
int main() {
    ...
    printf("== 工具清单 ==\n");
    {
        ...
        CHECK(call("nope", "{}").status != 0, "未知工具返回错误，不是空成功");
    }
    ...
    printf("== read ==\n");
    {
        ...
        CHECK(ok.status == 0 && ok.messages == "hello\nworld\n", "读到原文");

        const auto missing = call("read", R"({"file_path":)" + q(g_home / "nope.txt") + "}");
        CHECK(missing.status != 0 && missing.messages.find("cannot open") != std::string::npos,
              "文件不存在 → 报错，不是空内容");

        const auto noarg = call("read", "{}");
        CHECK(noarg.status != 0 && noarg.messages.find("file_path") != std::string::npos,
    ...
    printf("== edit（+x-0 = 创建） ==\n");
    {
        ...
        CHECK(created.status == 0 && created.messages.find("created") != std::string::npos,
              "文件不存在 → 创建（报的是 created，不是 appended）");
        CHECK(slurp(f) == "a\n", "创建的内容就是 new_string");

        const auto appended =
            call("edit", R"({"file_path":)" + q(f) + R"(,"old_string":"","new_string":"b\n"})");
        CHECK(appended.status == 0 && appended.messages.find("appended") != std::string::npos,
              "空 old_string → 追加");
        CHECK(slurp(f) == "a\nb\n", "追加在末尾，原内容不动");

        const auto edited =
            call("edit", R"({"file_path":)" + q(f) + R"(,"old_string":"a","new_string":"X"})");
        CHECK(edited.status == 0 && slurp(f) == "X\nb\n", "命中 → 只换第一处");

        const auto miss =
            call("edit", R"({"file_path":)" + q(f) + R"(,"old_string":"zzz","new_string":"X"})");
        CHECK(miss.status != 0 && miss.messages.find("not found") != std::string::npos,
              "old_string 找不到 → 报错");
        CHECK(slurp(f) == "X\nb\n", "报错时文件原样，没被写坏");

        const auto noarg = call("edit", R"({"file_path":)" + q(f) + "}");
        CHECK(noarg.status != 0 && noarg.messages.find("new_string") != std::string::npos,
              "缺 new_string → 报错");
    }
    ...
    printf("== read 的截断边界 ==\n");
    {
        ...
        CHECK(r_exact.messages.size() == 50000 && r_exact.messages.back() == 'x',
              "恰好读满 → 原样回传，不加省略号");
        CHECK(r_over.messages.size() == 50000 &&
                  r_over.messages.substr(r_over.messages.size() - 3) == "...",
              "真的更大 → 截断并标出来");
        CHECK(r_exact.messages != r_over.messages, "两种情况现在区分得开");
    }
    ...
    printf("== 权限裁决（一个配置键，一个 switch） ==\n");
    {
        ...
        CHECK(r.status == 0 && r.messages == "allow\n", "allow-all → 危险工具照跑");
    ...
    }
    {
        ...
        CHECK(r.status != 0 && r.messages.find("permission policy") != std::string::npos,
              "deny → 拒绝，理由说明是策略拒的");
        CHECK(asked == 0, "deny → 不问人（问了也白问）");

        const fs::path f = g_home / "r.txt";
        const auto ro = exe.execute("c3", "read", R"({"file_path":)" + q(f) + "}");
        CHECK(ro.status == 0, "deny 只管危险工具，read 照读");
    }
    {
        ...
        ap.set_emit([&ap, &seen_tool](const std::string& t, const std::string& payload) {
            ...
            const auto ev = json::parse(payload).value_or(json{});
            seen_tool = ev["tool"].as_string().value_or("");
            ap.respond(std::string(ev["id"].as_string().value_or("")), true);
        });
        CoreContext ctx{.config = &cfg, .emit_fn = nullptr};
        Executor exe(ctx, ap);
        const auto r = exe.execute("c4", "bash", R"({"command":"echo asked"})");
        CHECK(seen_tool == "bash", "ask → 发出 permission_request，点名是哪个工具");
        CHECK(r.status == 0 && r.messages == "asked\n", "用户放行 → 跑");
    }
    {
        Config cfg = config_with("ask");
        ApprovalCoordinator ap;
        ap.set_emit([&ap](const std::string& t, const std::string& payload) {
            if (t != "permission_request") return;
            const auto ev = json::parse(payload).value_or(json{});
            ap.respond(std::string(ev["id"].as_string().value_or("")), false);
        });
        CoreContext ctx{.config = &cfg, .emit_fn = nullptr};
        Executor exe(ctx, ap);
        const auto r = exe.execute("c5", "bash", R"({"command":"echo denied"})");
        CHECK(r.status != 0 && r.messages.find("denied by user") != std::string::npos,
    ...
    printf("== 中止（ADR-0002 R8） ==\n");
    {
        ...
        CHECK(after.interrupted && after.status != 0 && after.messages.find("should-not-run") == std::string::npos,
              "中止后紧接着的一次 execute 直接拒掉，命令没跑");

        exe.reset();
        const auto back = exe.execute("i2", "bash", R"({"command":"echo back"})");
        CHECK(back.status == 0 && !back.interrupted, "reset 抹掉中止痕迹，下一轮照常");
        ...
        CHECK(r.interrupted, "结果标出这次是被中止的（与工具自己失败不是一回事）");
    }
