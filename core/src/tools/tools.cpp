 ...
 * spawn.cpp / send_message.cpp），这里只把它们串起来。
...
const std::array<ToolDef, 5> &table()
{
    static const std::array<ToolDef, 5> k = {read_def(), edit_def(), spawn_def(),
                                             send_message_def(), bash_def()};
    return k;
}
