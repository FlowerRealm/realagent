...
配置树里那一个对象：装着[[端点束]]、凭证 `api_key`、这条路径上能用的模型清单 `models`，以及两档[[Model Tier]]各选中的 `model` / `small_model`。**盘上就这一份**——换后端就是改这一束的字段，没有「哪一份生效」这个问题，也就没有列表、没有切换、没有增删。

**两档的选择住在 provider 内部，不在外面。** 这一束是一起换的：端点、凭证、清单、选中的模型名同进同退。若把 `model` 留在外面，换完端点之后那个名字可能不在新清单里，于是要么静默发一个对面不认的模型名，要么补一段「换的时候校验并回落」的逻辑——两样都是特殊情况。

**它没有名字。** 名字是用来在多份里挑一份的，而这里只有一份。

**供应商身份不是一个概念**（ADR-0016）：core 认的是 [[Protocol]]，不是公司；没有任何一处代码能回答「对面是哪一家」，也不需要能。

_Avoid_: `backend`、`LLM client`、`profile`、`Provider 壳` / `套壳`（ADR-0004 那层带供应商知识的插件，死于 ADR-0016。同名不同物：这里的 Provider 是配置数据，没有代码、不兜底、不参与计价）

### **Model Tier（模型档位）**:

一次 LLM 调用用哪一档模型：**主模型**（`model`，对话主链路）或**小模型**（`small_model`，标题/摘要一类杂活）。档位只是模型名的选择，两档共用同一 base_url / api_key ——它们同属那一份 [[Provider]]，档位不构成第二份端点配置。两档选中的名字**存在 provider 内部**，候选出自它的 `models` 清单。`Config::model(ModelTier)` 解析档位，结果经 `dialog["model"]` 传给 `build_request`，协议层对档位无感。**不做档位间回落**：回落会让「我配了小模型」与「我没配所以用了主模型」长得一模一样，出账单时才发现区别。
...
`protocol` / `base_url` / `model` 三个键的合称，住在 [[Provider]] 那个对象里面（ADR-0023）——跟凭证 `api_key` 一起，换后端时一起换。

**它们没有默认值**（ADR-0017）：不填完全用不了，而填错产生的报错（端点 404、请求体形状不认、流解析出空）是最难自己诊断的一类——给默认值等于替用户猜，猜错了他还以为是自己配的。

**但没有人校验它们。** 「没有默认值」与「有人拦」是两件事，这里只成立前一件：core 不替你猜，也不替你拦。缺了就让它以**本来的方式**失败——空 `base_url` 换回 libcurl 一句 URL 格式错，端点不认的模型名换回一个 400，缺 `api_key` 换回一个 401。多一道校验就多一份会跟真正的失败漂移的真相。

**唯一的例外是 `protocol`**，因为它不失败：它决定请求长什么样，解不出来就没有「一个请求」可谈。所以解不出来时那一趟就地失败并说明只有哪三个值，**不是校验，是那一趟走不下去**。core 是常驻服务，一条消息不该杀掉它。

_Avoid_: `必需配置`、`必填校验`（不存在这东西——没有任何一处代码在开跑前检查配置齐不齐）、`默认端点`（也不存在）
...
同一个模型有多条到达路径（DeepSeek 直连 / OpenRouter / 内网中转），上下文窗口相同但端点、凭证、单价各异。这不构成冲突：**盘上只有一个 `base_url`**，表里配的就是这条路径的价。换路径就换 `base_url`，要换价就换表。

core 不校验用户配的模型名在不在表里——**表是参考资料，不是白名单**：配了表外的模型照发不误，不检查、不警告、不兜底（只是算不出钱）。`/model` 交互式选择是另一回事，那里只能从已知的里挑，而「已知的」出自 [[Provider]] 的 `models` 清单，不是这张表——**表回答「这个模型是什么货色」，清单回答「这条路径上有哪些模型」**，两个问题，两处数据。

_Avoid_: `模型清单`（指单个模型时）、`provider model`

### **模型数据表（models.json）**:

含单价在内的模型数据。**不参与候选**：`/model` 挑什么由 [[Provider]] 的 `models` 决定，这张表只负责给挑中的名字补上元数据与单价，查不到就只剩名字。两个来源，**不合并**：编译进二进制的出厂表随 core 升级更新；`~/.realagent/models.json` 是用户接管版，**存在即整表接管**，出厂表一概不看。半份表比没有表更难查，所以改一个模型的单价就得连表一起接管。
...
斜杠命令的第二层界面（参考 codex cli）：`/model` `/resume` `/statusline` `/provider` 无参执行后，用同一份结果载荷开一列东西——↑/↓ 换项、←/→ 移光标、Enter 确认、Esc 取消，模态（开着时按键只喂它）。面板不新增端点，数据就是命令回包里的 `data`；造不出面板（无数据/坏载荷）就退回文本清单。

**一列东西 + 一个高亮下标，就是全部数据结构。** 每一项要么是可选项，要么是可编辑的输入格（`panelItem.ed`，复用输入行那个 `editor`）——所以「选择清单」「二级菜单」「表单」是同一个东西的三种填法，不是三套机制，也不需要「选择态 / 编辑态」两个模式。

**提交只有一处**（发送、`awaiting`、面板续开都只写一遍），但载荷有两种形态（[[ADR-0023]] §6）：一条文本命令，或一份结构化 `data`。表单走后者——四个字段在客户端已经是分好的字符串，序列化成一行文本再解析回来只会凭空多出一套引号规则。
...
- **Provider** 拥有一个**端点束**、一个 `api_key`、一份模型清单，以及两档 **Model Tier** 各选中的名字；**盘上就这一份**，所以它不需要名字
- **Provider** 的 `models` 回答「这条路径上有哪些模型」，**模型数据表**回答「那个模型是什么货色」——`/model` 的候选出自前者，元数据与单价出自后者
